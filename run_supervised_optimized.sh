#!/bin/bash
# SLURM batch job script for supervised learning with optimization for HPC

#SBATCH -p gpu                     # Use gpu partition 
#SBATCH --gres=gpu:1               # Request 1 GPU (using 1 instead of 2 to conserve your hours)
#SBATCH -n 4                       # Request 4 CPU cores
#SBATCH --mem=32G                  # Request 32GB RAM (reduced from 48G)
#SBATCH --time=02:00:00            # Set time limit to 2 hours initially
#SBATCH -o supervised-opt-%j.out   # Output file name with job ID
#SBATCH -e supervised-opt-%j.err   # Error file name with job ID
...
echo "========================================================"
echo "Running Optimized Supervised Learning - $(date)"
echo "========================================================"
...
# Create a wrapper script to modify batch size and add progress reporting
cat > run_supervised_with_logging.py << 'EOF'
import sys
import os
...
# Override train method in SupervisedTrainer to add progress bar
def patched_train(self, data_paths):
    ...
    for epoch in range(self.config.num_epochs):
        ...
        # Do validation after every epoch for better monitoring
        ...
        # Logging
        print(f"Time: {time.time() - start_time:.2f}s | Loss: {self.history['train_loss'][-1]:.4f} | Val Dice: {val_dice:.4f}")
...
def main():
    ...
    parser.add_argument('--experiment_name', type=str, default='supervised_baseline',
    ...
    config.num_epochs = 30  # Reduced number of epochs for initial testing
...
# Run with smaller batch size explicitly set
echo "Running with batch size 2 for memory optimization and enhanced logging"
python3 $WORK_DIR/run_supervised_with_logging.py \
