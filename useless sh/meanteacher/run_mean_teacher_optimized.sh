#!/bin/bash
# SLURM batch job script for Mean Teacher SSL with optimization for HPC
...
#SBATCH -o mean_teacher-opt-%j.out  # Output file name with job ID
#SBATCH -e mean_teacher-opt-%j.err  # Error file name with job ID
...
echo "========================================================"
echo "Running Optimized Mean Teacher Learning - $(date)"
echo "========================================================"
...
# Create a wrapper script to modify and improve Mean Teacher training
cat > run_mean_teacher_with_logging.py << 'EOF'
import sys
import os
...
# Override train method in StableSSLTrainer to add progress bar
...
def main():
    ...
    parser.add_argument('--experiment_name', type=str, default='mean_teacher_ssl',
    ...
    # Create trainer
    trainer = StableSSLTrainer(config)
    
    # Apply patch to add progress bar
    trainer.train = patched_train.__get__(trainer, StableSSLTrainer)

    # Print experiment info
    print("\nExperiment Configuration:")
    print(f"Training Type: mean_teacher")
    ...
    with open(exp_dir / 'config.txt', 'w') as f:
        f.write(f"Training Type: mean_teacher\n")
...
echo "Running Mean Teacher with batch size 2 for memory optimization and enhanced logging"
python3 $WORK_DIR/run_mean_teacher_with_logging.py \
  --experiment_name mean_teacher_ssl \
  --data_dir $DATA_DIR \
  --batch_size 2
...
echo "========================================================"
echo "Results are located in: $WORK_DIR/ssl_results"
echo "========================================================"